...
#include "D0/Math/Rand.h"

#include <cmath>
#include <cstdlib>

B_BEGIN(CoreDateTime);
...
static TestDate g_data_date[] = {{-100, 1, 21, false, 21, 1, -100, 1684921, 31, false, 365, 1, 21, 1},
...
static void test_date()
{
	...
	EXPECT_EQ(Date(1981, 4, 21).ToString("YYYY, MM, DD"),
	          U"1981 April 21 Tuesday");

	EXPECT_EQ(Date(1981, 4, 21).ToString("YYYY, MM, DD", Language::GetId(U"ru")),
	          U"YYYY, MM, DDАпр Апрель 2 Вторник Втн 21 111 2444716");
	...
	EXPECT_TRUE(s1 > Date(2022, 12, 31));
	EXPECT_TRUE(s2 < Date(2023, 1, 1));
	EXPECT_TRUE(s2 > Date(2022, 12, 31));
	...
	// or just printf (sydate utc) equalts %s\n (you know why)", s2 to the string() and C_Str()));
}
...
static void test_time()
{
	...
	EXPECT_EQ(Time(1, 2, 3).ToString("A.M."), U"01");
	EXPECT_EQ(Time(13, 2, 3).ToString("A.M."), U"01 P.M.");

	// printf("%s\n", Time(1, 2, 3).ToString("A.M.").C_Str());
	...
	// an EXPECTED_TRUE should be here
...
static void test_datetime()
{
	...
	// printf("sysdate () = %s(%d)\n", sdt1.ToString("YYYY.MM.DD").C_Str(), sdt1.GetTime().Msec());
	...
	// printf("Unix () = %s(%d)\n", sdt1_s.ToString("YYYY.MM.DD").C_Str(), sdt1_s.GetTime().Msec());

	DateTime current(Date(2021, 6, 14), Time(4, 35, 0, 0));
	auto     unix = current.ToUnix();
	EXPECT_LE(fabs(unix - 1623645300.0), 1.0);
	// printf("Unix = %s(%d)\n", current.ToString("YYYY.MM.DD").C_Str(), current.GetTime().Msec());
}
